...
description: >
  ...
  Playfair Display in lowercase weight 900 treated as graphic primitive, IBM Plex Mono chrome (uppercase,
...
typography:
  # — reading ramp (Inter — narration/narrator voice) —
  body:    { fontFamily: "Inter", cqw: 1.2, weight: 400, lineHeight: 1.6, color: "cream" }
  lead:    { fontFamily: "Inter", cqw: 1.6, weight: 400, lineHeight: 1.5, color: "cream" }
  caption: { fontFamily: "Inter", cqw: 0.9, weight: 400, lineHeight: 1.5, color: "cream-muted" }
  label:   { fontFamily: "IBM Plex Mono", cqw: 0.72, weight: 500, tracking: "0.14em", upper: true }
  # — display / hero ramp (Playfair Display, sentence/title case, negative tracking) —
  h3:      { fontFamily: "Playfair Display", cqw: 2.8, weight: 700, lineHeight: 1.2 }
  quote-text:{ fontFamily: "Playfair Display", cqw: 3.8, weight: 700, lineHeight: 1.15, tracking: "-0.02em" }
  h2:      { fontFamily: "Playfair Display", cqw: 4.5, weight: 700, lineHeight: 1.1, tracking: "-0.02em" }
  stat-value:{ fontFamily: "Playfair Display", cqw: 5.5, weight: 700, lineHeight: 1.0, tracking: "-0.04em" }
  h1:      { fontFamily: "Playfair Display", cqw: 7.5, weight: 700, lineHeight: 0.9, tracking: "-0.03em" }
  fadelist-item:{ fontFamily: "Playfair Display", cqw: 7.5, weight: 700, lineHeight: 1.0, tracking: "-0.03em" }
  quote-mark:{ fontFamily: "Playfair Display", cqw: 10.0, weight: 700, lineHeight: 0.6 }
  fadelist-title:{ fontFamily: "Playfair Display", cqw: 10.5, weight: 700, lineHeight: 0.9, tracking: "-0.04em" }
  display: { fontFamily: "Playfair Display", cqw: 13.0, weight: 700, lineHeight: 0.88, tracking: "-0.04em" }
...
text and becomes graphic primitive.** Playfair Display `display` at 13cqw puts a single lowercase word
...
**Playfair Display** carries every text role from display to body — expressive range from weight (400–900)
...
- **Massive lowercase Playfair Display 900**, negative-tracked, as graphic primitive (display 13cqw).
...
Two ramps. The **reading ramp** (Playfair Display body 1.2cqw, mono label 0.72cqw) carries copy + chrome; the
**display ramp** (Playfair Display `h2` 4.5cqw → `display` 13cqw, weight 700–900) carries every statement.
...
- **Playfair Display display is lowercase, weight 700–900, negative-tracked** (−0.04em largest, −0.02em h2). **Mono chrome is uppercase, 0.1em+.** No italic, no underline, no uppercase display.
...
Playfair Display `display` (13cqw) lowercase in ink, left-anchored, over a small ink rule stub + mono kicker; a
Playfair Display lead line beneath in 75% ink. **Chrome** mono catalogue number top-left, mono meta top-right
...
**Ground** ink-black. **Composes** kicker, display. **Focal** a 2–4 word Playfair Display `display`/`h1`
...
row of three top-border-only stat-cards — big Playfair Display-900 numeral in `{colors.fire-orange}`, Playfair Display
...
**Ground** ink-black. **Composes** fadelist (3 stacked Playfair Display-900 words at 1.0/0.5/0.22), fadelist-title.
...
Playfair Display `quote-text` (700, lowercase) at ≤78cqw under an oversized fire-orange `quote-mark` (10cqw,
...
- Set every Playfair Display display in **lowercase weight 900**, negative-tracked — the system's signature.
...
- Never uppercase Playfair Display display; never add a second accent color.
...
- No serif companion; chrome is never Playfair Display.
...
- **Type** — Playfair Display lowercase 900 negative-tracked, fit-to-measure; mono chrome uppercase 0.14em; ≥1.4cqw floor.
...
- **Playfair Display + IBM Plex Mono via Google Fonts**; Noto Sans SC is the CJK fallback (the lowercase-display signal has no CJK equivalent — the two-register color system carries the identity, per the source).
